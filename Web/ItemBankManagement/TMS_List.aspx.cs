...
namespace Web.ItemBankManagement
{
    public partial class TMS_List :BasePage
    {
        ...
        private void gv01_Bind()
        {
            ...
            int CourseID = int.Parse(Label1.Text);
            ...
            if (rbl_Extent.SelectedValue.ToString() == "")
            {
                ...
                {
                    list = bll.GetList(Convert.ToInt32(ViewState["PageCount"]), PageSize, Request.QueryString["TitleType"], CourseID);
                }
            }
            else
            {
                if (CourseID == 0)
                {
                    list = bll.GetList(Convert.ToInt32(ViewState["PageCount"]), PageSize, Request.QueryString["TitleType"], rbl_Extent.SelectedValue.ToString());
                }
                else
                {
                    list = bll.GetList(Convert.ToInt32(ViewState["PageCount"]), PageSize, Request.QueryString["TitleType"], CourseID, rbl_Extent.SelectedValue.ToString());
                }
            }
        ...
        protected void gv01_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            if (bll.Del(ID) > 0)
            {
                Common.Function.AlertRedirect("删除成功！", "/ItemBankManagement/TMS_List.aspx?TitleType=" + Request.QueryString["TitleType"]);
            }
            else
            {
                Common.Function.AlertRedirect("系统繁忙！", "/ItemBankManagement/TMS_List.aspx?TitleType=" + Request.QueryString["TitleType"]);
            }
        }
